...
static void run(void)
{
	...
	SAFE_FILE_READ_STR(PATH_HOSTNAME, proc_val, sizeof(proc_val));
	TST_EXP_EQ_STR(name->nodename, proc_val);

	SAFE_FILE_READ_STR(PATH_OSRELEASE, proc_val, sizeof(proc_val));
	TST_EXP_EQ_STR(name->release, proc_val);

	SAFE_FILE_READ_STR(PATH_VERSION, proc_val, sizeof(proc_val));
	TST_EXP_EQ_STR(name->version, proc_val);

	SAFE_FILE_READ_STR(PATH_DOMAINNAME, proc_val, sizeof(proc_val));
